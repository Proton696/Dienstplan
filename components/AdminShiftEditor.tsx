    ...
    <>
      ...
      <div className="hidden md:block px-4 pb-6">
        <div className="glass rounded-3xl overflow-hidden shadow-apple">
          <table className="w-full">
            ...
            <tbody>
              {nonAdminRows.map((row) => (
                <tr key={row.employee.id} className="border-b border-white/[0.04] last:border-0 transition-colors">
                  <td className="px-5 py-3 rounded-xl transition-colors hover:bg-white/[0.02]">
                  ...
                  {weekDays.map((day) => {
                    ...
                    return (
                      <td
                        key={day.dateString}
                        className={clsx(
                          "px-2 py-2 text-center cursor-pointer rounded-xl transition-colors hover:bg-white/[0.02]",
                          justSaved && "bg-accent-green/10"
                        )}
                        onClick={() =>
                          setEditing({
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4 pb-safe">
          ...
          <div className="relative w-full max-w-xs bg-surface-secondary rounded-3xl shadow-apple-lg animate-slide-up overflow-hidden mb-10 md:mb-0">
